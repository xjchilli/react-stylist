...
class WardrobeModify extends Component{
    ...
    modify(){
        ...
        ToolDps.post('/wx/garderobe/modify',formdata,{'Content-Type':'multipart/form-data'}).then((data)=>{
            let obj={
                msgShow:true,
                msgText:'',
                uploadBtn:'确认上传',
     ...
     */
    delete(){
        ToolDps.post('/wx/garderobe/delete',{gid:this.state.gid}).then((res)=>{
            if(res.succ){
                this.setState({
                    msgShow:true,
                    msgText:'删除成功',
